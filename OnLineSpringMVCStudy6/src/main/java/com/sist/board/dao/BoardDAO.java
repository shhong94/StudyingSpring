...
public class BoardDAO {
	...
	public List<BoardVO> boardListData(int page)
	{
		...
		try
		{
			String sql="{CALL projectBoardListData3(?,?,?)}";
	...
	public void boardInsert(BoardVO vo)
	{
		try
		{
			String sql="{CALL projectBoardInsert3(?,?,?,?)}";
			cs=dbConn.getConn().prepareCall(sql);
	...
	public BoardVO boardDetailData(int no)
	{
		...
		try
		{
			String sql="{CALL projectBoardDetailData3(?,?)}";
			cs=dbConn.getConn().prepareCall(sql);
			cs.setInt(1, no);
	...
	public BoardVO boardUpdateData(int no)
	{
		...
		try
		{
			String sql="{CALL projectBoardUpdateData3(?,?)}";
			cs=dbConn.getConn().prepareCall(sql);
			// 실행 요청 ?에 값을 채운다 
			cs.setInt(1, no);
			// OUT => 저장 공간을 만들어 준다 
	...
	public boolean boardUpdate(BoardVO vo)
	{
		...
		try
		{
			String sql="{CALL projectBoardUpdate3(?,?,?,?,?,?)}";
			// 전송 => 오라클 
	...
	public boolean boardDelete(int no,String pwd)
	{
		...
		try
		{
			String sql="{CALL projectBoardDelete3(?,?,?)}";
			cs=dbConn.getConn().prepareCall(sql);
			cs.setInt(1, no);
			cs.setString(2, pwd);
	...
	public int boardTotalPage()
	{
		...
		try
		{
			String sql="SELECT boardTotalPage3() FROM project_board3";
